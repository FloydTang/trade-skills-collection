...
- `工具工作间/01_在整理的Skill/外贸skill源文件`：只作为开发源、验证源和内部维护源。
- `工具工作间/02_场景拆解/外贸skill`：展示课程工作间读取说明、两份资产同步入口、制作中节点和内部维护镜像。
- `半斤九两工具库/外贸skill`：成品层和课程工作间上游素材层。
- `半斤九两课程库`：用户可见课程内容和作战台同步内容的正式沉淀层。

已经达到可演示或可交付的 Skill，应先落到 `半斤九两工具库/外贸skill`，再交给课程工作间课程化。作战台不直接读取工具工作间或工具库原始目录；作战台同步的是课程库和课程化同步稿。
